...
async function GetList() {  
    let res = await PenguRequestAPI( 'GET','api/user/getall', {},  {},  true) .then(r => r.json()).catch(error => {console.log(error); return false})
    // if (list)
    //     movieList = list;
...
function RenderMovieList() {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (user) {
            ...
            item.find("#user-role").text(user.roles.map(r => r.name.replace(/^ROLE_/g, "")).join(", "))

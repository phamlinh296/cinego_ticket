//để không cho phép  truy cập vào trang đăng ký nếu đã đăng nhập
$(async function () {
...
$(".account-form").on("submit", async function (event) {
    ...
    let res = await PenguRequestAPI('POST', 'api/auth/signup',
        ...
        .then(r => r.json())
        .catch(error => { console.log("er=", error); return false })

    //----- ẩn then đi TEST res do be trả về---------
    // console.log("HTTP Status:", res.status);

    // let text = await res.text();  // Lấy nội dung thô
    // console.log("Raw Response Body:", text);//User  registered successfully

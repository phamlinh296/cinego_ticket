...
$(async () => {
    ...
    genreList = await PenguRequestAPI('GET', 'api/genre/getall', {}, {}, false)
        .then(r => r.json())
    ...
    let list = await PenguRequestAPI('GET', 'api/movie/getall', {}, {}, false)
        .then(r => r.json())
        // .then(r => {
        //     console.log("Response Object:", r); // Log đối tượng Response trước khi parse JSON
        //     return r.json();
        // })
        .catch(error => { console.log(error); return false })
    if (list)
...
$('#genre-container').on("click", ".genre-item", function (e) {
    ...
    // console.log("genreSelectsFilter=", genreSelectsFilter)//ok
...
function RenderMovieList() {
    ...
    for (let i = fromIndex; i < toIndex; i++) {
        ...
        if (movie) {
            ...
            item.find('a').attr('href', `movie-details.html?movie-id=${encodeURI(movie.id)}`)
